...
function login(req, res, next) {
    ...
    if (validator.isEmpty(password)) validationErrors.push('Password cannot be blank.');
...
async function signup(req, res, next) {
    ...
    const validationErrors = [];
    if (!validator.isEmail(email)) validationErrors.push('Please enter a valid email address.');
    if (!validator.isLength(password, { min: 8 })) validationErrors.push('Password must be at least 8 characters long');
    if (validationErrors.length) {
        res.status(400).send(validationErrors);
        return;
    }
